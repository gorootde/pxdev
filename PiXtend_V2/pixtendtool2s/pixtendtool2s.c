...
void help()
{
    printf("PiXtend Tool V2 -S- https://www.pixtend.de - Version %s\n", VERSION);
    ...
    printf("can be found at https://www.pixtend.de/downloads/\n");
...
int autoMode(int cyclephase) 
{
    ...
    //Read current hardware, uC Firmware Version and model
...
int main(int argc, char *argv[]) 
{
    ...
    if(argc > 1)
    {
        ...
        else if(strcmp(argv[1],"-do")==0 && argc==3)
        {
            ...
            //Do data transfer cycle 0 - First cycle
            RetVal = autoMode(0);           
            //Check for compatible PiXtend V2 -S- model
            if (isCompatibleModel() == -1){ errno = echoModelConflict(); return errno;}
            //Do data transfer cycle 1 - Second cycle
            RetVal = autoMode(1);
        ...
        else if(strcmp(argv[1],"-ucv")==0 && argc==2)
        {
            ...
            //Check return code, 0 = OK
            if (isReturnCodeOK(RetVal) == 0){
                printf("Microcontroller version is [%d.%d] on PiXtend V2 -S-\n", gHardware, gFirmware);
            }else{
                errno = RetVal;
            }
        }
